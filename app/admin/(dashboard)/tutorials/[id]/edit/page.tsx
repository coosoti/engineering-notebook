...
import { useState, useEffect, use } from "react" // Make sure 'use' is imported
import { useRouter } from "next/navigation"
import { useForm } from "react-hook-form"
import { updateTutorialAction } from "@/lib/actions/tutorials"
...
export default function EditTutorialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadTutorial() {
      try {
        const response = await fetch(`/api/tutorials/${id}`) // Use unwrapped id
  ...
  }, [body, id, getValues]) // Use unwrapped id in dependencies

  const onSubmit = async (data: any) => {
    setIsSubmitting(true)
    try {
      const result = await updateTutorialAction(id, { // Use unwrapped id
        ...data,
        body: body
      })
      ...
      alert(error.message || 'An error occurred while updating the tutorial')
    } finally {
